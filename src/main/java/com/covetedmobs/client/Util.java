...
public class Util {
	public static <T extends Block> void registerBlock(T block, String name, Material mat, SoundType sound, float hardness, float resistance, String tool, int level, String... oreDictionaryNames) {
		ResourceLocation loc = new ResourceLocation(CovetedMobs.MODID, name);
		block.setRegistryName(loc);
		block.setTranslationKey(loc.toString().replace(":", "."));
		block.setCreativeTab(CovetedMobs.tab);
	...
	public static <T extends Item> T registerItem(T item, String name, List<Predicate<ItemStack>> predicates, String... oreDictionaryNames) {
		...
		item.setCreativeTab(CovetedMobs.tab);
	...
	//		CovetedMobs.network.sendTo(new TeleportPlayerClient(x, y, z), (EntityPlayerMP) player);
	//	}
	//}
	...
	//	CovetedMobsAPI.ALTAR_UPGRADES.put(s -> s.getBlockState().getBlock() == ModObjects.placed_item && s.getTileEntity() instanceof TileEntityPlacedItem && OreDictionary.itemMatches(stack, ((TileEntityPlacedItem) s.getTileEntity()).getInventories()[0].getStackInSlot(0), stack.getHasSubtypes()), upgrade);
	//}
